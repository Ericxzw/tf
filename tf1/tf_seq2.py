'''
@Time : 2020/12/8 13:14
@Author : xzw
@File : tf_seq2.py
@Desc : 修改后能够运行在TensorFlow2.x环境的代码实现
'''
import sys
import os
import numpy as np
import tensorflow as tf
from tf1 import s2s as seq2seq
...
wordToken = word_token.WordToken()  # 调用分词
file_list = ['./samples/question', './samples/answer']

# 放在全局的位置，为了动态算出num_encoder_symbols和num_decoder_symbols
max_token_id = wordToken.load_file_list(file_list, min_freq)
...
def get_train_set():
    ...
    train_set = []
    with open(file_list[0], 'r', encoding='utf8') as question_file:
        with open(file_list[1], 'r', encoding='utf8') as answer_file:
            while True:
...
def get_model(feed_previous=False):
    ...
    cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(size)
